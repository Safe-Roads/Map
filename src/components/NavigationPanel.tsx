...
export default function NavigationPanel({
  ...
  const [waypoints, setWaypoints] = useState<
    { text: string; coords: [number, number] | null }[]
  >([]);
  const [isExpanded, setIsExpanded] = useState(false);
  const [travelMode, setTravelMode] = useState<"drive" | "bicycle" | "walk">(
    "drive",
  );
  ...
  const currentLocationLookupIdRef = useRef(0);

  const resolveAndSetCurrentLocationAddress = async (
    coords: [number, number],
  ) => {
    const [lat, lon] = coords;
    const lookupId = ++currentLocationLookupIdRef.current;

    setSourceCoords(coords);
    setSourceText("Locating...");

    if (!apiKey) {
      setSourceText(`${lat.toFixed(6)}, ${lon.toFixed(6)}`);
      return;
    }
    ...
    try {
      ...
      const formatted = data?.features?.[0]?.properties?.formatted as
        | string
        | undefined;

      // Ignore stale responses from previous requests.
      if (lookupId !== currentLocationLookupIdRef.current) return;

      setSourceText(formatted || `${lat.toFixed(6)}, ${lon.toFixed(6)}`);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => {
      if (activeInput?.type === "source")
        fetchSuggestions(sourceText, "source");
      else if (activeInput?.type === "dest") fetchSuggestions(destText, "dest");
      else if (
        activeInput?.type === "waypoint" &&
        activeInput.index !== undefined
      ) {
        fetchSuggestions(
          waypoints[activeInput.index].text,
          "waypoint",
          activeInput.index,
        );
      }
  ...
  const handleUseCurrentLocation = () => {
    onCurrentLocation();
    if (userLocation) {
      void resolveAndSetCurrentLocationAddress(userLocation);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (sourceCoords && destCoords) {
      ...
      const waypointCoords = await Promise.all(
        waypoints.map(async (w) => w.coords || (await geocode(w.text))),
      );
      const validWaypointCoords = waypointCoords.filter((c) => c !== null) as [
        number,
        number,
      ][];
      ...
      if (sCoords && dCoords) {
        ...
        alert(
          "Please select locations from the suggestions or be more specific.",
        );
      }
    }
  };
                        ...
                        >
                          {sourceSuggestions.map((s) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                handleSelectSuggestion(s, "source")
                              }
                  ...
                  <div key={idx} className="relative">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setWaypoints(newWps);
                        const newSugg = waypointSuggestions.filter(
                          (_, i) => i !== idx,
                        );
                        setWaypointSuggestions(newSugg);
                      }}
                          ...
                          >
                            {waypointSuggestions[idx].map((s) => (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  handleSelectSuggestion(s, "waypoint", idx)
                                }
                                className="w-full px-4 py-3 text-left text-sm hover:bg-blue-50 border-b border-gray-50 last:border-none flex items-start gap-3 transition-colors"
                              >
                                <MapPin className="w-4 h-4 text-gray-400 mt-0.5 flex-shrink-0" />
                  ...
                  <div className="flex items-center justify-between py-2 px-1">
                    <div className="flex items-center gap-2">
                      <ShieldCheck
                        className={cn(
                          "w-5 h-5",
                          avoidPotholes ? "text-green-600" : "text-gray-400",
                        )}
                      />
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto pr-1 custom-scrollbar">
                    {allRoutes.map((route, idx) => {
                      ...
                      const safetyScore = Math.max(
                        0,
                        100 - potholePenalty - signalPenalty,
                      );
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-[10px] text-gray-500 pt-2 border-t border-blue-100">
                    <div className="flex items-center gap-1">
                      <Clock className="w-3 h-3" />
                      <span>
                        Est. Travel Time: {formatTime(routeInfo.time)}
                      </span>
                    </div>
                  </div>
                  ...
                  {routeInfo.potholes > 0 && (
                    <div className="mt-3 flex items-start gap-2 text-xs text-red-700 bg-red-50 p-2 rounded-lg border border-red-100">
                      ...
                      <span>
                        Caution: {routeInfo.potholes} potholes detected on this
                        path. Drive carefully!
                      </span>
                    </div>
                  )}
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="space-y-1 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                      {routeInfo.instructions.map((inst, i) => (
                        <div
                          key={i}
                          className="flex gap-3 p-2 rounded-lg hover:bg-white transition-colors text-xs text-gray-700 border border-transparent hover:border-gray-100"
                        >
