...
const userLocationIcon = (heading: number | null) =>
  L.divIcon({
...
const destinationIcon = (apiKey: string) =>
  new L.Icon({
...
const sourceIcon = (apiKey: string) =>
  new L.Icon({
  ...
  const filteredPotholes = useMemo(() => {
    ...
    if (isNavigating && allRoutes[selectedRouteIndex]) {
      ...
      result = result.filter((p) => {
        ...
        return polyline.some((point) => {
          ...
          if (
            Math.abs(p.latitude - point[0]) > 0.0002 ||
            Math.abs(p.longitude - point[1]) > 0.0002
          )
            return false;
    ...
    // If a route is selected but not navigating, show potholes near the route (within 50m)
      ...
      result = result.filter((p) => {
        ...
        return polyline.some((point) => {
          if (
            Math.abs(p.latitude - point[0]) > 0.0005 ||
            Math.abs(p.longitude - point[1]) > 0.0005
          )
            return false;
    ...
    const fetchRoutes = async () => {
      ...
      try {
        ...
        if (data.features && data.features.length > 0) {
          const candidatePotholes = potholes.filter(
            (p) => p.status === "detected",
          );
          const newRoutes: Route[] = data.features.map(
            (feature: any, index: number) => {
              ...
              const nearby = candidatePotholes.filter((p) => {
                ...
                const isNear = sampledPolyline.some((point) => {
                  ...
                  if (
                    Math.abs(p.latitude - point[0]) > 0.0003 ||
                    Math.abs(p.longitude - point[1]) > 0.0003
                  )
                    return false;
    ...
    if (nearbyPothole) {
      ...
      if (!lastAlert || now - parseInt(lastAlert) > 30000) {
        ...
        const utterance = new SpeechSynthesisUtterance(
          `Caution! Pothole ahead. Severity level ${nearbyPothole.severity || 3}`,
        );
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        ...
        {isolineData && (
          <Polygon
            positions={isolineData.geometry.coordinates[0].map((coord: any) => [
              coord[1],
              coord[0],
            ])}
            pathOptions={{
            ...
            <Popup>
              <div className="p-2">
                ...
                <p className="text-xs text-gray-600">
                  This area is reachable within your selected time limit.
                </p>
                <button
                ...
                pathOptions={{
                  color:
                    index === selectedRouteIndex
                      ? segment.traffic === "high"
                        ? "#ef4444"
                        : segment.traffic === "medium"
                          ? "#f59e0b"
                          : avoidPotholes
                            ? "#10b981"
                            : "#3b82f6"
        ...
        {allRoutes[selectedRouteIndex] && (
          <>
            ...
            <Marker
              position={
                allRoutes[selectedRouteIndex].polyline[
                  allRoutes[selectedRouteIndex].polyline.length - 1
                ]
              }
